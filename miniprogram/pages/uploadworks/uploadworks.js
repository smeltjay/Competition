...
const works = db.collection('workInformation');
...
Page({
  ...
  commitInformation(){
    ...
    db.collection('workInformation').add({
      data:{
        workName: workName,
  ...
  uploadImage: function () {
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        ...
        // 上传图片
        const cloudPath = 'works/' + timestamp + filePath.match(/\.[^.]+?$/)[0]
        wx.cloud.uploadFile({
          cloudPath,
          filePath,
          success: res => {
            ...
            db.collection('workInformation').get({
              ...
              success: res => {
                ...
                number = list.length
                console.log(number)
                db.collection('fileIds').add({
                  data: {
                    ID:app.globalData.fileID,
                    path: app.globalData.imagePath,
                    number:number,
                    workType:workType,
                    workName: workName,
